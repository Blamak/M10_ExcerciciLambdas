...
//		List<String> alumnes = llistaAlumnes.stream()
		llistaAlumnes.stream().map(alumne -> alumne.getNom() + " - " + alumne.getEdat() + " anys.")
...
//		System.out.println(alumnes);
		 ...
		 * asigna-ho a un altre llistaAlumnes y mostra per pantalla la nova
		 * llistaAlumnes (tot amb lambdas)
		 */

		List<Alumne> nomPerA = llistaAlumnes.stream().filter(alumne -> alumne.getNom().charAt(0) == 'A')
		 ...
		 */
		List<Alumne> cincOMes = llistaAlumnes.stream().filter(alumne -> alumne.getNota() >= 5)
				.collect(Collectors.toList());
